...
TArray<USceneComponent*> APointCollection::GetGroundPoints(const FVector& GroundLocation, int32 NumPoints, float YawOveride) const
{
	...
	for (USceneComponent* Pt : ImmutablePts)
	{
		...
		if (Pt != Pt_0)
		{
			...
			ToPoint = ToPoint.RotateAngleAxis(YawOveride, FVector::UpVector);
		...
		GetWorld()->LineTraceSingleByProfile(HitResult, RaisedLocation, LowerLocation, "BlockAll", Params);
